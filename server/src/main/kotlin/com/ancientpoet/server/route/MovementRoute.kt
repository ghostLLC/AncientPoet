...
fun Route.movementRoute(movementService: MovementService) {
    authenticate("auth-jwt") {
        post("/user/location/{dynastyId}/move-preview") {
            val request = call.receive<MoveRequest>()
            call.respond(movementService.preview(call.userId(), call.parameters["dynastyId"]!!, request.toName, request.toLat, request.toLng))
        }
        post("/user/location/{dynastyId}/move") {
            ...
            call.respond(
                ...
                MovementResponse(
                    ...
                    movingToName = result.movingToName,
                    movingToLat = result.movingToLat,
                    movingToLng = result.movingToLng,
